...
-- assigment work class
select * from orders;
select customer_id,customer_name, order_date,row_number() over(partition by customer_id order by order_date) from orders;
-- Q2 
select customer_id,customer_name, amount,row_number() 
over( order by amount desc) from orders;

select order_id, category ,amount,rank() 
over( partition by category order by amount desc) as Ranking_by_amount from orders;
-- Q3
select customer_id,customer_name, amount,row_number() 
over( order by amount desc) from orders;

-- Q3 find the differnce between order amount and avg() amount of its category 

select order_id,category,amount,avg(amount)  over(partition by category) ,
round( amount - avg(amount)  over(partition by category),2) from orders;

-- Q4 find the highest maximum order every customers 
select * from  
(select customer_id,customer_name,order_id, amount,
max(amount) over(partition by customer_id) as max_amount from orders )as t
where amount = max_amount; 

-- # same question with rank functions Q4 and Q5 is same 
select * from  
(select customer_id,customer_name,order_id, amount,
rank () over(partition by customer_id order by amount desc)c_rank from orders )as t
where  c_rank =1; 

-- Q6 find the  top two highest value of every customers;
select * from  
(select customer_id,customer_name,order_id, amount,
rank () over(partition by customer_id order by amount desc)c_rank from orders )as t
where  c_rank <=2;
...
-- Q7 find the secound highest order every customers 
select * from  
(select customer_id,customer_name,order_id, amount,
rank () over(partition by customer_id order by amount desc)c_rank from orders )as t
where  c_rank =2; 

-- Q8 calculate the percentage contribution each order to customers total spending 
SELECT 
    *,
    SUM(amount) OVER(PARTITION BY customer_id) AS total_spending,
    (amount * 100.0) / SUM(amount) OVER(PARTITION BY customer_id) AS percentage_contribution
FROM orders;
