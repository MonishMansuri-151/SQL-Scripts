...
select City ,sum(Salary)  as total from Employees group by City having total > 600000;
-- Q17. Show roles with more than 7 employees.
select Role ,count(*) as c from Employees group by Role having c > 7;
-- Q18. Show regions where the average salary is below 70000
select Region , avg(Salary) as sal from Employees group by Region having sal < 70000;
...
select State , count(*) from Employees group by State having count(*) > 12;
...
select Region,Role,count(*) as c from Employees group by Region,Role;
-- Q25. Find max salary grouped by state AND department.
select State,Dept, max(Salary) as sal from Employees group by State,Dept;
...
select State,Role,count(*) from Employees group by State,Role having count(*) >1;
-- ▸ E. Derived Columns – YEAR / MONTH / QUARTER
...
select year(JoinDate),avg(Salary) from Employees group by year(JoinDate) order by year(JoinDate);
-- Q32. Count how many employees joined in each month (across all years).
select month(JoinDate),count(*) from Employees group by month(JoinDate) order by month(JoinDate);
-- Q33. Find total salary grouped by quarter of the join date.
select quarter(JoinDate),sum(Salary) from Employees group by quarter(JoinDate) order by quarter(JoinDate);
-- Q34. Count employees who joined in Q1 (Jan–Mar) versus Q3 (Jul–Sep).
select quarter(JoinDate) as quotes, count(*) from Employees group by quarter(JoinDate) having quotes = 1 or quotes = 3;
-- Q35. Show the year and department with headcount, ordered by year and dept.
select year(JoinDate) as y, Dept,count(*) from Employees group by y,Dept order by y,Dept;
-- Q36. Show years where more than 6 employees joined.
select year(JoinDate)as y, count(*)as c from Employees group by y having c >= 6 order by y ;
-- Q37. Find the average salary for employees joining in each quarter, show only quarters with avg salary above 70000.
select quarter(JoinDate)as q, avg(Salary) as s from Employees group by q having s > 70000 order by q;
-- ▸ F. City / State / Region 
-- Q38. Find the number of employees and average salary per state.
select count(*) , avg(Salary) as s from Employees group by State order by count(*); 
-- Q39. Find the department with highest headcount in each state.
select State, Dept , count(*) as c from Employees group by State, Dept order by c desc;
-- Q40. Show cities with more than 2 departments represented.
select City, count(distinct Dept) as d  from Employees group by City having d > 2;
...
select Region, count(*)as c,avg(Salary) from Employees group by Region having c > 10  and avg(Salary) > 67000;
...
select Dept,avg(Salary)as average from Employees where JoinDate > "2019-01-01" group by Dept; 
-- Q46. Find total salary per city for employees in the IT department only.
select City,Dept,sum(Salary) as sal from Employees where Dept = "IT"group by City;
-- Q47. Count employees per role for those with salary above 70000.
select Role ,count(*) from Employees where Salary > 70000 group by Role;
-- Q48. Show the max salary per department, only for employees in the South or West region.
select Dept ,max(Salary) from Employees where Region = 'South' or Region = "West" group by Dept ;
-- Q49. Count employees per year of joining for Texas (TX) state only.
select year(JoinDate), count(*) from Employees where State = 'TX'  group by year(JoinDate); 
-- Q50. Find departments where more than 3 female employees have salaries above 60000.
select Dept,count(Gender) as c from Employees where Gender = "F" and Salary > 60000  group by Dept having c > 3 ;
