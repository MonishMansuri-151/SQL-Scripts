...
-- with all opretor 
select * from employee as eout where dept_id =104 and
salary >=all(select salary from employee where dept_id= 104);
...
-- sub query 
-- sub query se ham sirf ek employee ki nikal paye agar ham 
-- correlated query sare department ki ek sath max salary nikal skate the the main use of correlated sub query  
select emp_name,salary,dept_id from employee 
where dept_id = 104 and  salary >=all (select max(salary) from employee where dept_id =104) ;
-- with correlated sub query 
-- remaninig he abhi
-- question 
-- ex departments having at least one  employee earning more than 70,000.

select dept_id, dept_name from department where 
dept_id =any (select dept_id from employee 
where salary > 70000);
-- question find secound-highest paid salry  employee in each department 
select emp_name , dept_id from employee as e  where salary = (select max(salary) from employee as e1 
